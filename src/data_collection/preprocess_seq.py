...
def process_sequences(input_csv, output_dir, window_size=7, ear_threshold=0.1050):
    ...
    # Loại bỏ các dòng trùng lặp về thời gian (vì ban đầu có dòng riêng cho left/right)
    df = df.drop_duplicates(subset=['video_id', 'frame_index']).copy()
    
    # Tạo mã thành viên để chuẩn hóa cá nhân
    ...
    # Chuyển đổi sang mảng Numpy phù hợp cho model LSTM/1D-CNN: shape (samples, timesteps, features)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input', type=str, default=r'd:\FPTU\SEM_4_[SUMMER_26]\DAP391m\project\DAP_GR3_SUM26\dataset\raw_eyes\metadata.csv', help='Đường dẫn file metadata gốc')
    parser.add_argument('--output', type=str, default=r'd:\FPTU\SEM_4_[SUMMER_26]\DAP391m\project\DAP_GR3_SUM26\dataset\train', help='Thư mục lưu X_train_seq.npy và y_train_seq.npy')
