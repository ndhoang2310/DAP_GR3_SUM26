...
3. **Nội suy phục hồi (Linear Interpolation):** Điền khuyết các frame bị rớt/mất bằng phương pháp nội suy tuyến tính cho EAR, và sao chép nhãn (Forward Fill) cho cột `final_label`.
4. **Cửa sổ trượt (Sliding Window):** Áp dụng cửa sổ kích thước $N = 7$ frames, bước trượt $step = 1$.
5. **Gán nhãn V-Shape (Dựa trên Ground-truth):** 
   - Sử dụng trực tiếp nhãn `final_label` (đã được con người kiểm duyệt) có sẵn trong tập dữ liệu để xác định chính xác trạng thái đóng/mở của mắt ở từng khung hình, loại bỏ sai số do sự khác biệt về kích thước mắt tự nhiên.
...
*(Cập nhật theo dữ liệu từ `dataset_master` sau khi áp dụng nhãn Ground-truth)*

- **Tổng số cửa sổ hợp lệ:** 3,996 mẫu
- **Nhãn 0 (No-Blink):** 3,220 mẫu
- **Nhãn 1 (Blink):** 348 mẫu
- **Nhãn 2 (Long-Closure):** 428 mẫu
