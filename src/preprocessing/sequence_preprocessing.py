...
def process_sequences(input_csv, output_dir, window_size=7):
    ...
    for video_id, group in df.groupby('video_id'):
        ...
        # Điền chuỗi (forward fill rồi backward fill) cho nhãn
    ...
    # Cắt cửa sổ trượt và dán nhãn V-Shape bằng ground-truth
    print(f"4. Đang trích xuất cửa sổ trượt (N={window_size}) và dán nhãn bằng final_label...")
    X = []
    y = []
    ...
    for video_id, group in df_interp.groupby('video_id'):
        ...
        label_values = group['final_label'].values # Dùng nhãn ground-truth để định nghĩa trạng thái
        ...
        for i in range(len(ear_norm_values) - window_size + 1):
            ...
            # LOGIC GÁN NHÃN V-SHAPE MỚI (Dùng ground-truth label)
            # Tìm các frame có nhãn là 'closed'
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Tiền xử lý chuỗi EAR (Sử dụng Ground-truth Labels)')
