...
def dynamic_normalize(x):
    """
    Chuẩn hóa Min-Max sử dụng cửa sổ mở rộng (expanding window).
    Mô phỏng thực tế: Ở frame thứ K, chỉ dùng thông tin từ frame 1 đến K.
    Tránh lỗi nhìn trộm tương lai (Future Leakage).
    """
    exp_min = x.expanding(min_periods=1).min()
    exp_max = x.expanding(min_periods=1).max()
    range_val = exp_max - exp_min
    
    # Nếu range = 0 (ví dụ frame đầu tiên), trả về 0.5 để tránh lỗi chia cho 0
    norm = np.where(range_val == 0, 0.5, (x - exp_min) / range_val)
